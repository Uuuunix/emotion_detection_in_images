...
应用访问运行 Flask 的计算机上的 0 号摄像头。macOS 弹出权限提示时，需要允许运行 Python 的应用访问摄像头。早期 BB-01 导航测试未启动摄像头，也未验证实际视频采集。
...
以下统计依据《测试用例清单.xlsx》中 `Test Cases测试用例` 工作表第 2～31 行，于 2026 年 9 月 14 日核对。数据来自清单中已记录的执行结果，本次 README 更新未重新运行测试。
...
| **合计** | **26 条白盒用例、4 条黑盒用例** | **30** | **22** | **7** | **1** |

严格通过率为 **22/30 = 73.33%**，失败率为 23.33%，部分通过率为 3.33%，无法执行（NT）占比为 0%。POK 和 pytest XFAIL 不计入验收通过数。

其中，`jyh-04` 和 `jyh-05` 的预期是复现当前 400/500 异常路径，因此记为 OK；`jyh-07` 和 `jyh-08` 要求对相应异常输入进行友好处理，因此记为 NG。验证现有行为通过，不代表该行为满足缺陷修复要求。

清单的基本信息页存在总数公式异常，显示了负数总量及相关比例。上表按 30 条有效明细及其状态重新统计，未采用异常汇总公式的结果。

### 自动化情况与脚本对应关系

清单中 15 条用例明确标记为自动化、4 条标记为非自动化，11 条 KY 用例的自动化字段为空。目前总用例数已达到 30 条要求；按已填写字段可确认的自动化比例为 **15/30 = 50%**。
...
仓库还包含清单之外的测试，因此测试运行器发现的用例数量不一定等于表中的 30 条。
...
本地 `outputs/blackbox-01-04-20260911/` 保存早期 BB-01～BB-04 的导航、初始状态、单脸上传和格式检查结果。这些编号与场景不同于清单中的 `jyh-bb-01`～`jyh-bb-04`，未合并计入上述 30 条。

`outputs/test-report-20260913/` 中的早期报告仅覆盖 11 条用例，尚未按本次汇总清单更新。这两个目录已被 Git 忽略，新克隆的仓库不包含这些本地材料。

原 6 条 WL 白盒用例对 `detect_faces_and_emotions` 达到了 100% 语句覆盖率和分支覆盖率。该覆盖率记录早于新增失败用例，不能代表清单中全部 30 条用例、整个 `app.py` 或端到端功能的覆盖情况。
...
- **人脸误检**：风景图、猫图产生误报，对应 `jyh-bb-02`；早期格式测试中也出现过背景物体被当成人脸的情况。
